                ...
                <div class="col-8 d-flex align-items-center border-top p-1 ">
                    ...
                    <div class="border border-0 bg-info px-2" style="border-radius: 30px;">
                        <a href="{{ route('posts.upvote', ['user_id' => session()->get('id'), 'post_id' => $post->id]) }}"><i
                                ...
                                class="fas fa-arrow-alt-circle-down ms-1"></i> {{ count($post->downvotes) }} </a>
                    </div>
                    @else
                    <div class="border border-0 bg-info px-2" style="border-radius: 30px;">
                        <a href="{{ route('posts.upvote', ['user_id' => -1, 'post_id' => $post->id]) }}"><i
                                class="fas fa-arrow-alt-circle-up me-1"></i>{{ count($post->upvotes) }} <i
                                class="text-muted"></a>|<a href="{{ route('posts.downvote', ['user_id' => -1, 'post_id' => $post->id]) }}"></i><i
                                class="fas fa-arrow-alt-circle-down ms-1"></i> {{ count($post->downvotes) }} </a>
                    </div>
                    @endif
